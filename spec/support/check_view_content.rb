module CheckViewContent
  ...
  def check_search_first_content_for(post)
  ...
  def check_search_last_content_for(post)
    expect(all('.card-body').last).to have_content post.title
    expect(all('.card-body').last).to have_content user.nickname
    expect(all('.card-body').last.find('.post-reputation-front')).to have_content check_content_star(post_all_contents.reputation)
  end

  # マイページの項目が正常に描画されていること
  def check_myPage_last_content_for(post)
